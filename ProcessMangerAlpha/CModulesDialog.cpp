...
void CModulesDialog::getModulesInfoByProcID(CString ProcID)
{
	...
	for (unsigned int i = 0; i < dwModeleCount; i++) {
		...
		objModulesList.InsertItem(i, _T(""));
		CString mp;
		mp.Format(_T("%s"), modulesPath);
		objModulesList.SetItemText(i, 0, mp);
		CString entryPoint;
		entryPoint.Format(_T("0x%x"), mofi.EntryPoint);
		objModulesList.SetItemText(i, 1, entryPoint);
		CString baseOfDll;
		baseOfDll.Format(_T("0x%x"), mofi.lpBaseOfDll);
		objModulesList.SetItemText(i, 2, baseOfDll);
		CString sizeImage;
		sizeImage.Format(_T("0x%x"), mofi.SizeOfImage);
